...
export default function DashboardLayout({ children, activeTab, setActiveTab, capital, reserve, totalEquity, apiStatus, userEmail, kospi, kosdaq }: LayoutProps) {
  const navItems = [
        ...
        <header className="h-auto py-4 md:h-20 border-b border-gray-200 flex flex-col md:flex-row items-start md:items-center justify-between px-4 md:px-10 sticky top-0 bg-gray-50/80 backdrop-blur-sm z-10 gap-4 md:gap-0">
          <div className="flex flex-col gap-1.5">
            ...
            <div className="flex gap-3">
              {kospi ? (
                <div className="flex items-center gap-1.5 font-mono text-[11px] bg-white/60 border border-gray-200 px-2 py-0.5 rounded">
                  <span className="text-gray-500 uppercase">KOSPI</span>
                  <span className="font-bold text-gray-900">{kospi.price?.toLocaleString('ko-KR', { maximumFractionDigits: 2 })}</span>
                  <span className={`font-bold ${(kospi.changePercent ?? 0) >= 0 ? 'text-green-600' : 'text-red-500'}`}>
                    {(kospi.changePercent ?? 0) >= 0 ? '▲' : '▼'}{Math.abs(kospi.changePercent ?? 0).toFixed(2)}%
                  </span>
                </div>
              ) : (
                <div className="font-mono text-[11px] bg-white/60 border border-gray-200 px-2 py-0.5 rounded text-gray-400">
                  KOSPI 로딩 중...
                </div>
              )}
              {kosdaq ? (
                <div className="hidden md:flex items-center gap-1.5 font-mono text-[11px] bg-white/60 border border-gray-200 px-2 py-0.5 rounded">
                  <span className="text-gray-500 uppercase">KOSDAQ</span>
                  <span className="font-bold text-gray-900">{kosdaq.price?.toLocaleString('ko-KR', { maximumFractionDigits: 2 })}</span>
                  <span className={`font-bold ${(kosdaq.changePercent ?? 0) >= 0 ? 'text-green-600' : 'text-red-500'}`}>
                    {(kosdaq.changePercent ?? 0) >= 0 ? '▲' : '▼'}{Math.abs(kosdaq.changePercent ?? 0).toFixed(2)}%
                  </span>
                </div>
              ) : null}
            </div>
