...
app.get("/api/market/index", async (req, res) => {
  try {
    ...
    const kospi = kospiResult.status === 'fulfilled' && kospiResult.value ? {
      price: (kospiResult.value as any).regularMarketPrice ?? null,
      change: (kospiResult.value as any).regularMarketChange ?? null,
      changePercent: (kospiResult.value as any).regularMarketChangePercent ?? null,
    } : null;

    const kosdaq = kosdaqResult.status === 'fulfilled' && kosdaqResult.value ? {
      price: (kosdaqResult.value as any).regularMarketPrice ?? null,
      change: (kosdaqResult.value as any).regularMarketChange ?? null,
      changePercent: (kosdaqResult.value as any).regularMarketChangePercent ?? null,
    } : null;

    res.json({ kospi, kosdaq });
  } catch (e: any) {
    res.status(500).json({ error: "지수 조회 실패: " + e.message });
  }
});
